...
[Returning a File using FileResult in ASP.NET Core MVC](https://geeksarray.com/blog/aspnet-core-mvc-returning-file-using-fileresult).

**Start here for this article:** `Controllers/DemoController.cs` — `DownloadBytes` returns an in-memory CSV as an attachment (check the Content-Disposition header), `DownloadStream` streams without buffering.
